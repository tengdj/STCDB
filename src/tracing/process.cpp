...
//                }
                ...
                for(uint j = 0;j<bench->config->SSTable_count;j++) {
                    Point *mid_points = new Point[bench->SSTable_kv_capacity/2+100];
                    int count_p = 0;
                    for (int i = 0; i < bench->SSTable_kv_capacity; i=i+2) {
                        box temp_box(bench->h_values[offset + bench->MemTable_count][j*bench->SSTable_kv_capacity+i]);
                        mid_points[count_p].x = temp_box.low[0] / 2 + temp_box.high[0] / 2;
                        mid_points[count_p].y = temp_box.low[1] / 2 + temp_box.high[1] / 2;
                        count_p++;
                    }
                    print_points(mid_points, bench->SSTable_kv_capacity/2);
                    delete[] mid_points;
                }
