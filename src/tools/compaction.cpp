...
float calculateActiveLength(const set<pair<float, float>>& activeIntervals) {
    ...
    for (const auto& interval : activeIntervals) {
        float y1 = interval.first, y2 = interval.second;
        if (y1 > prevY) {
            totalLength += y2 - y1;
        } else {
            if (y2 > prevY) {
                totalLength += y2 - prevY;
            }
        }
        prevY = max(prevY, y2);
    }
...
float calculateUnionArea(const vector<f_box>& rectangles) {
    ...
    for (const auto& event : events) {
        ...
        if (event.isStart) {
            ...
            activeIntervals.erase({event.y1, event.y2});
        }
...
void * merge_dump(workbench * bench, uint start_ctb, uint merge_ctb_count, vector< vector<key_info> > &keys_with_sid, vector< vector<f_box> > &mbrs_with_sid,
    ...
    q << "GB" << ',' << "STR(ms)" << ',' << "encoding" << ',' << "dump_keys" << ',' << "dump_all"
    << ',' << "area_exclusive" << ',' << "union_area" << ',' << "area_handshake" << ',' << "area_sum" << endl;
...
#pragma omp parallel for num_threads(bench->config->num_threads)
    ...
    q << merge_ctb_count * 2 << ',' << STR_time << ',' << encoding_time
      << ',' << load_keys_time << ',' << dump_C_time << ',' << area_exclusive << ',' << union_area << ',' << area_handshake << ',' << area_sum << endl;
    q.close();
